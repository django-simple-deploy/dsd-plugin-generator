...
def test_deployment(tmp_project, cli_options, request):
    """Test the full, live deployment process to {{PlatformName}}."""

    # Cache the platform name for teardown work.
    request.config.cache.set("platform", "{{PluginName}}")

    print("\nTesting deployment to {{PlatformName}} using the following options:")
