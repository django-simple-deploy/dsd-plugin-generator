"""Integration tests for django-simple-deploy, targeting NewFly."""
...
def test_settings(tmp_project):
    """Verify there's a NewFly-specific settings section.
...
def test_log_dir(tmp_project):
    ...
    assert "INFO: Configuring project for deployment to NewFly..." in log_file_text

    assert "INFO: CLI args:" in log_file_text
    assert (
        "INFO: Deployment target: NewFly" in log_file_text
        or "INFO: Deployment target: NewFly" in log_file_text
    )
    ...
    assert (
        "INFO: --- Your project is now configured for deployment on NewFly ---"
        in log_file_text
    )
