...
- Push these changes to NewFly.
- Open your deployed project in a new browser tab.
"""

cancel_newfly = """
Okay, cancelling NewFly configuration and deployment.
"""

# DEV: This could be moved to deploy_messages, with an arg for platform and URL.
cli_not_installed = """
In order to deploy to NewFly, you need to install the NewFly CLI.
  See here: ...
After installing the CLI, you can run the deploy command again.
"""

cli_logged_out = """
You are currently logged out of the NewFly CLI. Please log in,
...
def success_msg(log_output=""):
    ...
    msg = dedent(
        f"""
        --- Your project is now configured for deployment on NewFly ---
        ...
        - Push your project to NewFly's servers:
            $ ...
        - Open your project:
            $ ...    
...
def success_msg_automate_all(deployed_url):
    ...
    msg = dedent(
        ...
        --- Your project should now be deployed on NewFly ---
        ...
        If you make further changes and want to push them to NewFly,
