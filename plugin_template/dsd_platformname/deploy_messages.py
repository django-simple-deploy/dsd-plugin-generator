...
cancel_{{PlatformNameLower}} = """
Okay, cancelling {{PlatformName}} configuration and deployment.
"""

# DEV: This could be moved to deploy_messages, with an arg for platform and URL.
cli_not_installed = """
In order to deploy to {{PlatformName}}, you need to install the {{PlatformName}} CLI.
  See here: ...
After installing the CLI, you can run the deploy command again.
"""

cli_logged_out = """
You are currently logged out of the {{PlatformName}} CLI. Please log in,
...
def success_msg(log_output=""):
    ...
    msg = dedent(
        f"""
        --- Your project is now configured for deployment on {{PlatformName}} ---
        ...
        - Push your project to {{PlatformName}}'s servers:
            $ ...
        - Open your project:
            $ ...    
...
def success_msg_automate_all(deployed_url):
    ...
    msg = dedent(
        ...
        --- Your project should now be deployed on {{PlatformName}} ---
        ...
        If you make further changes and want to push them to {{PlatformName}},
